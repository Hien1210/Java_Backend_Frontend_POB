...
| `/shop/topping-categories` | [QuanLyLoaiToppingServlet.java](src/main/java/org/example/controllers/QuanLyLoaiToppingServlet.java) | [ToppingCategoryDAO.java](src/main/java/org/example/daos/ToppingCategoryDAO.java)/[ToppingCategoryDAOImpl.java](src/main/java/org/example/daos/ToppingCategoryDAOImpl.java) | [ToppingCategory.java](src/main/java/org/example/models/ToppingCategory.java) (cột DB: `name`, `description`, `is_deleted`, `category_id` NULL FK tới `Categories` — không có `status`) | [shop/Quanlyloaitopping.jsp](src/main/web/shop/Quanlyloaitopping.jsp) — dropdown chọn "loại sản phẩm" áp dụng (để trống = mọi loại), lọc thực tế ở `shop/Banhang.jsp` (POS) khi chọn topping cho từng dòng giỏ hàng — xem mục 60 trong CRUD_DA_LAM.md |
