...
| `/shop/bills` | [ShopBillServlet.java](src/main/java/org/example/controllers/ShopBillServlet.java) | OrderDAO (findByShopId, assignShipper), AccountDAO (findOnlineShippers), ShopDAO + BillUtil, [PdfExportUtil.java](src/main/java/org/example/utils/PdfExportUtil.java), [ExcelExportUtil.java](src/main/java/org/example/utils/ExcelExportUtil.java) | Order, Shop, BillView, Account | [shop/Quanlybill.jsp](src/main/web/shop/Quanlybill.jsp) — mỗi dòng đơn hàng có tọa độ hiển thị nút "📍" mở modal bản đồ Leaflet chỉ đọc (marker giao hàng + marker shop); quy trình đơn hàng đầy đủ `PENDING -> CONFIRMED -> READY_FOR_PICKUP -> (gán shipper thủ công hoặc shipper tự nhận) -> SHIPPING -> DONE` (xem mục 50 trong CRUD_DA_LAM.md); nút "📄 PDF" xuất hóa đơn (`?action=exportPdf&id=`), nút "📊 Xuất Excel (doanh thu)" (`?action=exportExcel`, xem mục 58), [shop/HoaDonShop.jsp](src/main/web/shop/HoaDonShop.jsp) — có sẵn bản đồ Leaflet chỉ đọc dưới phần "Địa chỉ giao hàng" (marker giao hàng + marker shop, tự `fitBounds`), ẩn nếu đơn hàng chưa có tọa độ |
...
| `/shop/topping-categories` | [QuanLyLoaiToppingServlet.java](src/main/java/org/example/controllers/QuanLyLoaiToppingServlet.java) | [ToppingCategoryDAO.java](src/main/java/org/example/daos/ToppingCategoryDAO.java)/[ToppingCategoryDAOImpl.java](src/main/java/org/example/daos/ToppingCategoryDAOImpl.java) | [ToppingCategory.java](src/main/java/org/example/models/ToppingCategory.java) (cột DB: `name`, `description`, `is_deleted`, không có `status`; quan hệ nhiều-nhiều với `Categories` qua bảng `ToppingCategory_ProductCategories`) | [shop/Quanlyloaitopping.jsp](src/main/web/shop/Quanlyloaitopping.jsp) — checkbox chọn NHIỀU "loại sản phẩm" áp dụng (không chọn gì = mọi loại), lọc thực tế ở `shop/Banhang.jsp` (POS) khi chọn topping cho từng dòng giỏ hàng — xem mục 60, 61 trong CRUD_DA_LAM.md |
...
| `/admin/bao-cao-van-hanh` | [BaoCaoVanHanhServlet.java](src/main/java/org/example/controllers/BaoCaoVanHanhServlet.java) (roleId=1) | [BaoCaoVanHanhDAO.java](src/main/java/org/example/daos/BaoCaoVanHanhDAO.java)/[BaoCaoVanHanhDAOImpl.java](src/main/java/org/example/daos/BaoCaoVanHanhDAOImpl.java), [ExcelExportUtil.java](src/main/java/org/example/utils/ExcelExportUtil.java) | - (query truc tiep `Orders`, `Order_Logs` qua SQL `GROUP BY`) | [admin/BaoCaoVanHanh.jsp](src/main/web/admin/BaoCaoVanHanh.jsp) — bo loc theo ngay, 3 the thong ke (tong don, ty le hoan thanh, thoi gian giao TB), Doughnut Chart.js trang thai don hang (xem muc 45 trong CRUD_DA_LAM.md), nut "Xuất thống kê (Excel)" (`?action=exportExcel`, xem muc 58) |
