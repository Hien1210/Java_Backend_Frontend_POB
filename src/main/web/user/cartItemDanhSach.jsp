...
<head>
    <meta charset="UTF-8">
    <title>Quan ly chi tiet gio hang</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1>Chi tiet gio hang</h1>
        <a href="${pageContext.request.contextPath}/cart-items?action=new" class="btn btn-primary">+ Them moi</a>
    </div>

    <c:if test="${param.success eq 'created'}"><div class="alert ok">Tao chi tiet gio hang thanh cong.</div></c:if>
    <c:if test="${param.success eq 'updated'}"><div class="alert ok">Cap nhat chi tiet gio hang thanh cong.</div></c:if>
    <c:if test="${param.success eq 'deleted'}"><div class="alert ok">Xoa chi tiet gio hang thanh cong.</div></c:if>
    <c:if test="${param.error eq 'not_found'}"><div class="alert err">Khong tim thay chi tiet gio hang.</div></c:if>

    <c:choose>
        <c:when test="${empty cartItemList}">
            <div class="empty">Chua co chi tiet gio hang nao.</div>
        </c:when>
        <c:otherwise>
            <table>
                <thead>
                ...
                <c:forEach var="item" items="${cartItemList}">
                    <tr>
                        ...
                        <td>
                            <div class="actions">
                                <a href="${pageContext.request.contextPath}/cart-items?action=edit&id=${item.id}" class="btn btn-warning">Sua</a>
                                <a href="${pageContext.request.contextPath}/cart-items?action=delete&id=${item.id}"
                                   class="btn btn-danger"
                                   onclick="return confirm('Xoa chi tiet gio hang #${item.id}?')">Xoa</a>
                            </div>
                        </td>
                    </tr>
                </c:forEach>
