...
<head>
    ...
    <title>Thùng rác - Loại topping</title>
    <style>
...
<main class="main-content">
    <header class="top-header">
        <h2>🗑️ THÙNG RÁC — LOẠI TOPPING</h2>
    ...
    <div class="content-wrapper">
        ...
        <c:if test="${param.success eq 'restore'}">
            <div class="alert alert-success">✅ Khôi phục loại topping thành công!</div>
        </c:if>
        ...
        <div class="back-bar">
            <a href="${pageContext.request.contextPath}/shop/topping-categories" class="btn-back">← Quay lại danh sách</a>
            <span class="trash-hint">🗑️ Các loại topping đã xóa — bấm <strong>Khôi phục</strong> để đưa trở lại danh sách.</span>
        </div>

        <div class="panel">
            <div class="panel-header">
                <span class="panel-title">Loại topping đã xóa</span>
                <span class="count-chip">${fn:length(deletedCategories)} mục</span>
            </div>
                ...
                <c:choose>
                    <c:when test="${empty deletedCategories}">
                        <div class="empty-state">
                            ...
                            <p>Thùng rác trống — không có loại topping nào đã xóa.</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <table>
                            ...
                            <tbody>
                                <c:forEach var="cat" items="${deletedCategories}" varStatus="vs">
                                    <tr>
                                        ...
                                        <td>${not empty cat.description ? cat.description : '—'}</td>
