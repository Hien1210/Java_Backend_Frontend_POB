...
<main class="main-content">
    ...
    <div class="content-wrapper">
        ...
        <c:if test="${param.success eq 'restore'}">
            <div class="alert alert-success">✅ Khôi phục topping thành công!</div>
        </c:if>
        ...
        <div class="back-bar">
            <a href="${pageContext.request.contextPath}/shop/toppings" class="btn-back">← Quay lại danh sách</a>
            <span class="trash-hint">🗑️ Các topping đã xóa — bấm <strong>Khôi phục</strong> để đưa trở lại danh sách.</span>
        </div>

        <div class="panel">
            <div class="panel-header">
                <span class="panel-title">Topping đã xóa</span>
                <span class="count-chip">${fn:length(deletedToppings)} mục</span>
            </div>
            <div class="table-wrap">
                <c:choose>
                    <c:when test="${empty deletedToppings}">
                        <div class="empty-state">
                            <div class="empty-icon">🗑️</div>
                            <p>Thùng rác trống — không có topping nào đã xóa.</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                        <table>
                            ...
                            <tbody>
                                <c:forEach var="t" items="${deletedToppings}" varStatus="vs">
                                    <tr>
                                        <td>${vs.index + 1}</td>
                                        <td><strong>${t.toppingName}</strong></td>
