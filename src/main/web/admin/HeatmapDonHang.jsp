...
<script>
    ...
    // Default center to Vietnam (Ho Chi Minh City / Hanoi area)
    ...
    if (heatmapPoints && heatmapPoints.length > 0) {
        ...
        orderDetails.forEach(function(item) {
            ...
            var formattedPrice = Number(item.amount).toLocaleString('vi-VN') + 'đ';

            var popupHtml =
                '<div class="popup-order-card">' +
                '  <div class="popup-order-title">' +
                '    <span>Đơn #' + item.id + '</span>' +
                '    <span style="color:#22c55e;">' + formattedPrice + '</span>' +
                '  </div>' +
                '  <div class="popup-order-info"><strong>Shop:</strong> ' + escapeHtml(item.shop) + '</div>' +
                '  <div class="popup-order-info"><strong>Địa chỉ:</strong> ' + escapeHtml(item.address || 'Chưa rõ địa chỉ') + '</div>' +
                '  <div class="popup-order-info" style="font-size:11px;color:#94a3b8;margin-top:4px;">🕒 ' + escapeHtml(item.time) + '</div>' +
                '</div>';

            marker.bindPopup(popupHtml);
            markersCluster.addLayer(marker);

            // Group by Shop or Region
            var groupName = extractGroupKey(item.address, item.shop);
            if (!regionMap[groupName]) {
                regionMap[groupName] = { count: 0, points: [] };
            }
            regionMap[groupName].count++;
            regionMap[groupName].points.push([item.lat, item.lng]);
        });
